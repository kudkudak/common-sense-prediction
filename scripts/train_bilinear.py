...
Run like: python scripts/train_argsimpp.py results/ArgSim++/OMCS --embeddings=/u/jastrzes/l2lwe/data/embeddings/LiACL/embeddings_OMCS.txt
...
def train(save_path, embeddings=os.path.join(DATA_DIR, "commonsendata/embeddings.txt")):
    ...
    #
    # score = Lambda(score_argsim_fnc, output_shape=(1,))(Ax)
    def identity3d(shape):
        return K.repeat(K.eye(shape[1]), shape[0]).transpose((1, 0, 2))
    score = Bilinear(len(V_rel), kernel_initializer=identity3d)([Ax, relation])

    clf = Dense(len(V_rel) if use_rel else 1, kernel_initializer="ones", \
        bias_initializer=constant(np.float32(-threshold_argsim)))(score)
    ...
    model.fit_generator(ds,
        ...
        callbacks=[EarlyStopping(patience=10, monitor="acc"),
